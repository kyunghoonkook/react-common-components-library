  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <CollapsibleTrigger asChild>
          <button style={{ borderRadius: '9999px', padding: '0.5rem', cursor: 'pointer' }}>
            {isOpen ? '접기' : '펼치기'}
          </button>
        </CollapsibleTrigger>
      </div>
      ...
      <CollapsibleContent style={{ marginTop: '0.5rem' }}>
        <div style={{ borderRadius: '0.375rem', backgroundColor: '#f9fafb', padding: '1rem' }}>
          <p>이 컴포넌트 라이브러리는 React 애플리케이션을 위한 재사용 가능한 UI 컴포넌트 모음입니다.</p>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
      {items.map((item) => (
        <Collapsible 
          ...
          open={openItem === item.id} 
          onOpenChange={(open) => setOpenItem(open ? item.id : null)}
          style={{ border: '1px solid #e5e7eb', borderRadius: '0.375rem', padding: '0.5rem' }}
        >
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <h3 style={{ fontSize: '1rem', fontWeight: 500 }}>{item.title}</h3>
            <CollapsibleTrigger asChild>
              <button style={{ borderRadius: '9999px', padding: '0.25rem', cursor: 'pointer' }}>
                {openItem === item.id ? '▲' : '▼'}
              </button>
            </CollapsibleTrigger>
          </div>
          
          <CollapsibleContent style={{ marginTop: '0.5rem', paddingTop: '0.5rem', borderTop: '1px solid #e5e7eb' }}>
            <p>{item.content}</p>
          </CollapsibleContent>
        </Collapsible>
      ))}
