...
import { HoverCard, Avatar, Button } from 'react-components-library';
...
export const BasicExample = () => {
  return (
    <div>
      <HoverCard
        trigger={<span style={{ color: '#6366f1', fontWeight: 'bold' }}>@홍길동</span>}
        content={
...
```jsx
import { HoverCard } from 'react-components-library';
