...
import { Tooltip, Button } from 'react-components-library';
import TooltipExample from '../../../src/components/tooltip/TooltipExample';

# Tooltip

Tooltip은 요소에 마우스를 올리거나 클릭했을 때 추가 정보를 표시하는 작은 팝업 컴포넌트입니다.
...
```jsx
import { Tooltip } from 'react-components-library';
```
