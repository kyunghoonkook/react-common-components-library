...
import { Avatar, AvatarGroup } from 'react-common-components-library';

# Avatar

사용자 프로필 이미지 또는 이니셜을 표시하는 컴포넌트입니다. 다양한 크기와 상태를 지원합니다. 라이트/다크 테마를 자동으로 지원합니다.
...
```jsx
import { AvatarGroup, Avatar } from 'react-common-components-library';
...
아바타에 테두리를 추가할 수 있습니다:

```jsx
import { Avatar } from 'react-common-components-library';

function AvatarBorderExample() {
  return (
    <div style={{ display: 'flex', gap: '16px', backgroundColor: '#f0f0f0', padding: '16px' }}>
      <Avatar 
        src="https://i.pravatar.cc/150?img=1" 
        alt="테두리 있는 아바타" 
        border={true}
...
<div className="component-preview" style={{ backgroundColor: 'var(--component-bg)' }}>
  <div style={{ display: 'flex', gap: '16px', padding: '16px', borderRadius: '8px', backgroundColor: 'var(--component-border)' }}>
    <Avatar 
      src="https://i.pravatar.cc/150?img=1" 
      alt="테두리 있는 아바타" 
      border={true}
